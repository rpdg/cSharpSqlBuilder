...
namespace Lyu.Data.Helper
{
	...
	/// </summary>
	public class MySql: BaseHelper
	{
		bool inTrans;
		StringBuilder TransactionSql;
		List<MySqlParameter> TransactionParamList;
		...
		{
			dac = new MySqlClient(connStr);
		}
		
		//使用某个数据库某个表
		public MySql(string connStr, Table tb)
			: this(connStr)
		{
			this.Use(tb);
		}
		...
		private BaseHelper BeginTrans()
		{
			...
			TransactionParamList = new List<MySqlParameter>();
			
			return this;
		}
		
		private BaseHelper SetTrans(string sql, MySqlParameter[] param)
		{
		...
		private BaseHelper CommitTrans()
		{
			...
			MySqlParameter[] param = TransactionParamList.ToArray();
		...
		public override BaseHelper Add(Dictionary<string , object> obj)
		{
			...
			foreach (KeyValuePair<string, Column> kv in cols) {
				...
				if (obj.ContainsKey(fieldName)) {
					...
					MySqlParameter param = makeParam(col, obj[fieldName]);
				...
				//obj中不存在数据，但在映射中存在默认值
					...
					if (col.Default.IndexOf("(", StringComparison.Ordinal) > -1) {
						...
						MySqlParameter param = makeParam(col, col.Default);
						_parameters.Add(param);
						_valueNames.Append(param.ParameterName + ",");
					}
				}
		...
		public override BaseHelper Set(Dictionary<string , object> updated, Dictionary<string , object> wheres)
		{
			...
			foreach (KeyValuePair<string, Column> kv in cols) {
				...
				if (updated.ContainsKey(colName)) {
					...
					MySqlParameter param = makeParam(col, updated[colName]);
					_parameters.Add(param);
			...
			MySqlParameter[] paramx = new MySqlParameter[n + m];
		...
		private MySqlParameter makeParam(Column col, object val)
		{
			string pName = "@" + col.Name + (++ParamCount);	
			MySqlDbType type = (MySqlDbType)Enum.Parse(typeof(MySqlDbType), col.Type, true);
			
			MySqlParameter param = new MySqlParameter(pName, type);
			param.Value = val;
